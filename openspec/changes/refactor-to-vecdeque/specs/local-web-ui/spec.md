...
The system MUST render UI with Askama templates using `data-*` attributes compatible with Datastar. The image-stack template MUST support full-stack render and single-entry render paths that produce equivalent markup for the same entry data.
...
- **WHEN** an image-stack entry is rendered via full-stack template and via single-entry template path
